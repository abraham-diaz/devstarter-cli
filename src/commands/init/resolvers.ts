 ...
 */
export function resolveTemplateFlag(
  optionTemplate: string | undefined,
  available: string[],
): string | undefined {
  if (!optionTemplate) return undefined;
  if (available.includes(optionTemplate)) return optionTemplate;
  throw new Error(
    `Invalid --template "${optionTemplate}". Available: ${available.join(', ')}`,
  );
}
 ...
 */
export function resolveTemplateFinal(
  ...
  if (templates.length === 0) return 'basic';

  // Prioridad 3: Modo defaults o único template
  if (useDefaults || templates.length === 1) return templates[0];
